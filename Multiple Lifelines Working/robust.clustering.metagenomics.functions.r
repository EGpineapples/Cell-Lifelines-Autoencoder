...
#   minClus: minimum number of clusters to check (integer).
#   maxClus: maximum number of clusters to check (integer).
plot.robust.clustering.all.together.formatted <- function(eval.array4d,minClus=2,maxClus=10){
  library(grid)  
  library(gridExtra)  
  #library(cowplot)  
  ...
  for (alg in c('pam','hclust')){
    for (score in c('SI','PS','Jaccard')){
      assFrame <- NULL
      assFrame <- data.frame(k=minClus:maxClus, JSD=eval.array4d[,score,'jsd',alg], rJSD=eval.array4d[,score,'rjsd',alg], BrayCurtis=eval.array4d[,score,'bray',alg], MorisitaHorn=eval.array4d[,score,'horn',alg], Kulczynski=eval.array4d[,score,'kulczynski',alg])
      ...
      if(score=='SI'){
        gp <- (ggplot(assFrame, aes(x=k, y=score, color=metric)) +
                 ...
                 scale_x_continuous(breaks=minClus:maxClus, labels=minClus:maxClus) +
...
#   minClus: minimum number of clusters to check (integer).
#   maxClus: maximum number of clusters to check (integer).
plot.robust.clustering.onlyPAM.formatted <- function(eval.array4d,minClus=2,maxClus=10){
  library(grid)  
  library(gridExtra)  
  #library(cowplot)  
...
#   minClus: minimum number of clusters to check (integer).
#   maxClus: maximum number of clusters to check (integer).
plot.robust.clustering.onlyPAM.formatted.title <- function(eval.array4d,title,minClus=2,maxClus=10){
  library(grid)  
  library(gridExtra)  
  #library(cowplot)  
...
#   minClus: minimum number of clusters to check (integer).
#   maxClus: maximum number of clusters to check (integer).
plot.robust.clustering.onlyHCLUST.formatted <- function(eval.array4d,minClus=2,maxClus=10){
  library(grid)  
  library(gridExtra)  
  #library(cowplot)  
...
#   eval.array4d: 4-dimensions array with clustering results, returned by robust.clustering() function.
#   title: text to appear in the top of the graphs.
#   minClus: maximum number of clusters to check (integer).
#   maxClus: maximum number of clusters to check (integer).
plot.robust.clustering.onlyHCLUST.formatted.title <- function(eval.array4d,title,minClus=2,maxClus=10){
  library(grid)  
  library(gridExtra)  
  #library(cowplot)  
...
############################################
### Function plot.barplot.species        ###
############################################
# Plot a barplot showing the phylum distribution in different clusters
plot.barplot.species <- function(data.phyloseq){
  data.df <- data.phyloseq  %>%
    # agglomerate at phylum level
    ...
    # Sort data frame alphabetically by phylum
    arrange(Phylum)   
  pdf('barplot_clusters_AggregatedBySpecies.pdf')
  p <- ggplot(data.df, aes(x=cluster, y=Abundance, fill=Species)) + 
    geom_bar(aes(), stat="identity", position="fill") +
    ylab("Relative Abundance")
  print(p)
  dev.off()
} # end-function plot.barplots.species
...
#   x 9 (k=minClus(2):maxClus(10)) potential number of clusters
...
robust.clustering <- function(data.norm=NULL,var.color=NULL,label=NULL,minClus=2,maxClus=10){
  # Load libraries
  ...
  iniSeed <- 12345
  # 2 clustering algorithms x 9 (k=minClus(2):maxClus(10)) potential number of clusters x 5 distance measures x 3 assessment scores
  ...
  eval.array4d <- array(0,dim=c(maxClus-minClus+1,3,5,2),dimnames=list(as.character(seq(minClus,maxClus,1)), list('SI','PS','Jaccard'), list('jsd','rjsd','bray','horn','kulczynski'), list('pam','hclust')))
  ...
  for (distanceMethod in c("jsd", "rjsd", "bray", "horn", "kulczynski")){
    ...
    for (alg in c("pam","hclust")){
      ...
      switch(alg,
             ...
             pam={ out.pred.str <- prediction.strength(dist, Gmin=minClus, Gmax=maxClus, M=100, clustermethod=claraCBI, classification="centroid", distances=TRUE) },
             hclust={ out.pred.str <- prediction.strength(dist, Gmin=minClus, Gmax=maxClus, M=100, clustermethod=disthclustCBI,method="average", classification="averagedist", distances=TRUE) }
      ) #end-switch-alg
      eval.array4d[,score,distanceMethod,alg] <- out.pred.str$mean.pred[minClus:maxClus]
      ...
      switch(alg, # Switch computes 'k' and 'si' for different algorithms
             pam={
               fit <- pamk(dist,krange=minClus:maxClus,diss=TRUE)
               k <- fit$nc
               if(k==1){
                 si <- 0 
               }else{
...
# criteria:
#   a.- to select the “k” with the highest average SI (in minClus-maxClus) in 2 measures
...
robust.clustering.decision <- function(data.now=NULL,eval.array4d=NULL,var.color=NULL,label=NULL,minClus=2,maxClus=10){
  thresh.SI <- 0.25
  thresh.PS <- 0.80
  thresh.Jac <- 0.75
  ...
  avgSI <- array(0,dim=c(maxClus-minClus+1,ncol(combi)),dimnames=list(as.character(seq(minClus,maxClus,1)), as.character(seq(1,ncol(combi),1))))
  ...
  if((SI.ok)&&((PS.ok)||(Jac.ok))){
    ...
    plot.barplot.species(data.norm)
...
robust.clustering.all.steps <- function(path,RDataOrBiomFile,dataset.label,variable.in.PCoAgraph,taxaSubsetDominant='all',percDominant=1,taxaSubsetGenus='no',mapBiomFile,minClus=2,maxClus=10){
  # Load libraries
  library(ggplot2,quietly=TRUE)
  ...
  array4d.results <- robust.clustering(data.norm,variable.in.PCoAgraph,dataset.label,minClus,maxClus)
  
  # B) PLOT GRAPHS
  load(paste('eval.arrays_',dataset.label,'.RData',sep=''))
  plot.robust.clustering.all.together.formatted(eval.array4d,minClus,maxClus)
  
  # C) AUTOMATIC CLUSTERING DECISION
  data.norm <- robust.clustering.decision(data.norm,eval.array4d,variable.in.PCoAgraph,dataset.label,minClus,maxClus)
