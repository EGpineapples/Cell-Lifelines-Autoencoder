...
def create_autoencoder_timesteps(input_shape, lstm_units, lstm_units_layer2, dense_units, dropout_rate):
    ...
    encoded = Dropout(dropout_rate)(encoded)
    latent_space = LSTM(dense_units, activation='tanh', return_sequences=True)(encoded)
    decoded = LSTM(lstm_units_layer2, activation='tanh', return_sequences=True)(latent_space)
...
def objective(trial, train_data, val_data, config):
    ...
    model = create_autoencoder_timesteps(train_data.shape[1:], lstm_units, lstm_units_layer2, dense_units, dropout_rate)
    model.compile(optimizer=Adam(learning_rate=learning_rate), loss='mse')

    early_stop = EarlyStopping(monitor='val_loss', patience=config['model']['patience'], restore_best_weights=True)
    model.fit(train_data, train_data, epochs=config['model']['epochs'], batch_size=batch_size, validation_data=(val_data, val_data), callbacks=[early_stop], verbose=1)
...
def train_and_optimize_autoencoder(normalized_data, config):
    ...
    best_model = create_autoencoder_timesteps(train_data.shape[1:], best_params['lstm_units'], best_params['lstm_units_layer2'], best_params['dense_units'], best_params['dropout_rate'])
    ...
    model_save_path = 'src/models/autoencoder_tuned.h5'
...
def save_latent_representations(model, train_data, file_path):
    latent_representations = model.predict(train_data)
    latent_df = pd.DataFrame(latent_representations.reshape(-1, latent_representations.shape[-1]))
