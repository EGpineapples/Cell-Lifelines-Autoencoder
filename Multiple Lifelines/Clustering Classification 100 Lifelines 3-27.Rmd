...
latent_data_file <- "D:/Desktop/Work/Multiple Lifelines/latent_space_representations_100lifelines_378timesteps.csv"

# Read the CSV file
latent_df <- read.csv(latent_data_file, header = TRUE, check.names = FALSE)
...
tsne_results <- Rtsne(latent_df_random)

# Extract the first two dimensions from the t-SNE results
tsne_dim1 <- tsne_results$Y[, 1]
tsne_dim2 <- tsne_results$Y[, 2]

# Add the t-SNE dimensions back to the data frame with the cluster assignments
# df$tsne_dim1 <- tsne_dim1
# df$tsne_dim2 <- tsne_dim2

# Plot all clusters together using ggplot2 for better visuals
ggplot(df, aes(x = tsne_dim1, y = tsne_dim2, color = as.factor(cluster))) +
  geom_point() +
  labs(title = "t-SNE Clusters", x = "Dimension 1", y = "Dimension 2") +
  theme_minimal() +
  scale_color_discrete(name = "Cluster")

# Save the plot if needed
ggsave("all_clusters_tsne.png", width = 10, height = 8)

# To find which lifelines belong to which clusters, you can look at the 'labels' dataframe
# If you need to save this information to a file, you can use write.csv
 write.csv(labels, "lifelines_to_clusters_timesteps.csv", row.names = TRUE)
...
```
```{r}
# Assuming you've already performed PCA and have pca_results
pca_results <- prcomp(latent_df_random, scale. = TRUE)

# Extract the proportion of variance explained
prop_var_explained <- summary(pca_results)$importance[2, ]

# Calculate cumulative variance explained
cum_var_explained <- cumsum(prop_var_explained)

# Find the number of components needed to reach 90% variance explained
num_comp_90_var <- which(cum_var_explained >= 0.9)[1]

# Create a data frame for plotting
variance_df <- data.frame(
  PC = seq_along(cum_var_explained),
...
# Create the plot
p <- ggplot(variance_df, aes(x = PC, y = CumulativeVariance)) +
  geom_line(size = 1) +
  geom_point(size = 3) +
  geom_hline(yintercept = 0.9, linetype = "dashed", color = "red", size = 1) +
  geom_vline(xintercept = num_comp_90_var, linetype = "dashed", color = "red", size = 1) +
  scale_x_continuous(breaks = seq(0, length(cum_var_explained), by = 2)) +
  scale_y_continuous(labels = scales::percent_format(scale = 1), limits = c(0, 1)) +
  labs(title = "PCA Analysis", x = "# of Features", y = "% Variance Explained") +
  annotate("text", x = num_comp_90_var, y = 0.05, label = paste(num_comp_90_var, "features"), vjust = 0, color = "red") +
